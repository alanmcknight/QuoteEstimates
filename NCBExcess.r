...
Data = data.frame(matrix(NA, nrow = 35100, ncol = 8))
colnames(Data) <- c("Vehicle.Group", "Postcode.Group", "Age", "NCB", "Total.Excess", "Price", "Total.Excess.Group", "NCB.Group")

Data[, 1] = ceiling((1:35100)/702)
Data[, 2] = 0
Data[, 3] = ceiling((1:702)/9)+16
Data[, 4] = ceiling((1:9)/(3))
Data[, 5] = 1:3
...
A <- gam(Policy.Price ~ s(Vehicle.Group)+s(Age)+s(No.Claim.Bonus)+s(Excess), data = a, family=Gamma(link=log))
Data$Price = predict(A, newdata = data.frame(Age=Data$Age, Vehicle.Group=Data$Vehicle.Group, No.Claim.Bonus=Data$NCB.Group, Excess=Data$Total.Excess.Group), type="response")

#names(Data)[4] <- "NCB.Group"
write.table(Data[,1:6], file=paste(z, "NCBExcess.csv", sep=""), row.names=F, sep=",")
Data$Price <- 0
}

Data[, 2] = 99
A <- gam(Policy.Price ~ s(Vehicle.Group)+s(Age)+s(No.Claim.Bonus)+s(Excess), data = d, family=Gamma(link=log))
Data$Price = predict(A, newdata = data.frame(Age=Data$Age, Vehicle.Group=Data$Vehicle.Group, No.Claim.Bonus=Data$NCB.Group, Excess=Data$Total.Excess.Group), type="response")

#names(Data)[4] <- "NCB.Group"

write.table(Data[,1:6], "NoPCNCBExcess.csv", row.names=F, sep=",")