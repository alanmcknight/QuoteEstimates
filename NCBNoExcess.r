...
A <- gam(Policy.Price ~ s(Vehicle.Group)+s(Age)+s(No.Claim.Bonus), data = a, family=Gamma(link=log))
Data$Price = predict(A, newdata = data.frame(Age=Data$Age, Vehicle.Group=Data$Vehicle.Group, No.Claim.Bonus=Data$NCB.Group), type="response")

write.table(Data[,1:6], file=paste(z, "NCBNoExcess.csv", sep=""), row.names=F, sep=",")
Data$Price <- 0
}

Data[, 2] = 99
A <- gam(Policy.Price ~ s(Vehicle.Group)+s(Age)+s(No.Claim.Bonus), data = d, family=Gamma(link=log))
Data$Price = predict(A, newdata = data.frame(Age=Data$Age, Vehicle.Group=Data$Vehicle.Group, No.Claim.Bonus=Data$NCB.Group), type="response")


write.table(Data[,1:6], "NoPCNCBNoExcess.csv", row.names=F, sep=",")